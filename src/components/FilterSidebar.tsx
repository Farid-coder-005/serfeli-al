    ...
    <div className="bg-white rounded-[2rem] border border-gray-100 p-7 shadow-sm space-y-2">
      ...
      <div className="pb-4 border-b border-gray-50 mb-2 flex items-center justify-between">
        ...
        <button 
          ...
          className="text-[#1E293B] hover:text-[#FF6B00] transition-colors p-2 rounded-lg hover:bg-orange-50"
          title="Sıfırla"
        >
      ...
      <div className="py-4">
        <h3 
          className="text-[10px] font-[900] text-[#1E293B] mb-6 uppercase tracking-[0.2em]"
          style={{ fontFamily: "'Montserrat', sans-serif" }}
        >
          Qiymət Aralığı (₼)
        </h3>
        ...
        <div key={group.id} className="border-t border-gray-50 pt-1">
          ...
          >
            ...
            {openSections.includes(group.id) ? (
              <ChevronDown className="w-4 h-4 text-[#1E293B] group-hover:text-[#FF6B00]" />
            ) : (
              <ChevronRight className="w-4 h-4 text-[#1E293B] group-hover:text-[#FF6B00]" />
            )}
          </button>
            ...
            <div className="space-y-3 pb-4 animate-in fade-in slide-in-from-top-2 duration-300">
              {group.options.map((option) => (
                ...
                >
                  ...
                  <span className="text-[10px] font-bold text-[#1E293B] group-hover:text-[#FF6B00] transition-colors">
                    {option.count}
                  </span>
                </label>
              ))}
